...
func RegisterHandlers(r *routing.RouteGroup, service Service, authHandler routing.Handler, logger log.Logger) {
	...
	r.Post("/api/feedback", res.postFeedback)
	...
	r.Get("/api/feedbacks", res.getFeedback)
...
func (r resource) postFeedback(c *routing.Context) error {
	var input CreateFeedBack
	...
	var reqToken = strings.Split(c.Request.Header.Get("Authorization"), "Bearer ")[1]
	...
	feedBack, err := r.service.CreateFeedBack(c.Request.Context(), input, owner)
	if err != nil {
		return err
	}

	return c.WriteWithStatus(feedBack, http.StatusCreated)

}

func (r resource) getFeedback(c *routing.Context) error {
	...
	if claims, ok := token.Claims.(jwt.MapClaims); ok {
		...
		Feedback, err := r.service.getFeedback(c.Request.Context())
		//fmt.Println(profile.TypeSports)
		if err != nil {
			return err
		}
		return c.Write(Feedback)
